...
def _count_circular_blobs(binary: np.ndarray) -> int | None:
    ...
    for c in contours:
        a = cv2.contourArea(c)
        # Loosened size thresholds: allow slightly smaller and larger blobs
        if a < total * 0.002 or a > total * 0.18:
            continue
        p = cv2.arcLength(c, True)
        # Relax circularity threshold to accept more imperfect/foreshortened pips
        if p > 0 and (4 * np.pi * a / p ** 2) >= 0.28:
            count += 1
...
def _try_set_manual_exposure(cap: cv2.VideoCapture, exposure: float, auto_mode: float) -> float:
    cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, auto_mode)
...
def set_manual_exposure(cap: cv2.VideoCapture, exposure: float) -> None:
    ...
    if platform.system() == "Windows":
        ...
        modes = [1.0, 3.0, 0.25, 0.75]

    requested = float(exposure)
    actual = None
    for mode in modes:
        actual = _try_set_manual_exposure(cap, requested, mode)
        if abs(actual - requested) < 0.1:
            return

    # If the exact requested value is not accepted, leave the last attempt and log it.
    print(
        f"Warning: requested exposure={requested}, last applied exposure={actual}, "
        f"auto_exposure_mode_attempts={modes}"
    )
