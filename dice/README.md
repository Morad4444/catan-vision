...
```bash
pip install -r src/dice/requirements.txt
```

3. Start the app from the repository root:

```bash
python src/dice/app.py
```
...
```bash
python src/dice/app.py --camera-index 1
```

If Windows exposes the Logitech device under a slightly different name, set it explicitly:

```bash
python src/dice/app.py --camera-name "Logitech HD Webcam C270"
```
